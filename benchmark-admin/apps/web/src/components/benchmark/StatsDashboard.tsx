...
export function StatsDashboard() {
  ...
  return (
    <section aria-label="统计" className="space-y-2 rounded-md border border-[hsl(var(--border))] p-3 text-sm">
        ...
        <table className="w-full text-xs">
          ...
          <tbody>
            {data.groups.map((g: StatsGroup) => (
              <tr key={`${g.shotType}|${g.questionType}`} className="border-t border-[hsl(var(--border))]">
                <td className="py-1">{g.shotType || '—'}</td>
                <td className="py-1">{g.questionType || '—'}</td>
