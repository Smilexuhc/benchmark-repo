...
export const aiRouter = t.router({
  ...
  extractFields: protectedProcedure.input(ExtractFieldsInput).mutation(async ({ input }) => {
    const data = await ai.extractFields(input.kind, input.description, input.options);
    if (input.kind === 'character')
      return { kind: 'character' as const, data: data as CharacterData };
    if (input.kind === 'scene') return { kind: 'scene' as const, data: data as SceneData };
    return { kind: 'prop' as const, data: data as PropData };
  }),

  generateImage: protectedProcedure.input(GenerateImageInput).mutation(async ({ input }) => {
    ...
    if (input.refImage !== undefined) {
      const [refImg] = await db
        ...
        .from(media)
        .where(and(eq(media.id, input.refImage), isNull(media.deletedAt)))
        .limit(1);
      if (refImg) {
