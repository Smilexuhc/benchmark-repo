...
export const PROP_EXTRACT_SYSTEM = `你从用户的中文道具描述里提取结构化字段，只输出一个 JSON 对象，不要任何多余文字。
字段（值均用中文，描述未提及的填空字符串 ""）：
- category 类别：尽量从给定「候选值」中选最贴切的一个；候选里确实没有合适的再自拟简短词。
- description 概述、title 标题：简短中文短语。`;
...
export function buildPropExtractUserMessage(
  description: string,
  options?: Record<string, string[]>,
): string {
  if (!description.trim()) throw new Error('自由描述为空');
