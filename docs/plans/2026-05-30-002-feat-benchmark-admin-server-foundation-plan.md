...
│       │                                 #   + raw routes (auth login/logout, export zip; multipart upload retired BEN-25)
│       └── package.json
├── packages/
...
**`mediaAssetsRouter`:** `list` query — `{ kind?, mediaType?, dedup? }` → `AssetImageOut[]` (joins `asset_images`→`assets`, optional dedup by `object_key`, R12). `getUploadUrl` mutation — `{ mediaType: 'image' | 'audio' | 'video', filename: string }` → `{ uploadUrl, objectKey }`. **Server-authoritative**: the lowercase extension parsed from `filename` is matched against `services/upload/validate.ts`'s `EXT_TO_CONTENT_TYPE` allowlist; a miss or a `mediaType` whose family disagrees with the resolved content-type (e.g. `mediaType=image` with `foo.mp3`) → `BAD_REQUEST`. The `contentType` passed to `getPresignedPutUrl` comes from the table, never from the client — clients cannot ask the server to sign a PUT for an arbitrary MIME type. Size is bounded by the TOS bucket quota; no `ContentLength` is added to the presigned URL.
...
- **Approach:** Login constant-time-compares posted email/password against `ADMIN_EMAIL`/`ADMIN_PASSWORD`; on success sets an http-only, `SameSite=Strict`, `Secure` cookie. **Token (explicit, parent U8):** `base64url(payload).base64url(HMAC-SHA256(payload, SESSION_SECRET))`, `payload = { jti, iat, exp }`, `jti` a random 128-bit nonce, TTL ~4h (tightened from 12h — see Accepted risks). Verify recomputes the HMAC constant-time, rejects on mismatch or expiry. **Logout/revocation:** `POST /api/auth/logout` clears the cookie and adds `jti` to a bounded in-memory revocation set (entries auto-expire at `exp`; single-process state is fine — one long-lived host, KTD-4). **CSRF:** `SameSite=Strict` plus an `x-trpc-source` header check on mutations as defense-in-depth. **Rate-limit (in scope):** per-IP attempt cap with backoff on `/api/auth/login`. Context reads the cookie, verifies, populates `session`; `protectedProcedure` throws `UNAUTHORIZED` when absent/forged/expired/revoked.
  ...
  - **Shared `requireSession` preHandler:** the cookie verify+populate logic is factored into one named Fastify preHandler (`requireSession`) exported from `auth/`, reused by every session-gated raw route (logout, export zip; the multipart upload route was retired in BEN-25) **and** mirrored by the tRPC `protectedProcedure`. One implementation, one place to audit — raw routes must not hand-roll their own cookie check.
- **Raw-route auth matrix** (the routes that bypass tRPC context — each must declare its guard):
  ...
  | ~~`POST /api/upload` (multipart)~~ | **Decommissioned (BEN-25, 2026-06-05).** Uploads now go presigned-PUT via the `mediaAssets.getUploadUrl` tRPC procedure — the sole upload contract. |
...
- **Approach:**
  ...
  - **Upload** — **Decommissioned in BEN-25 (2026-06-05).** The server-mediated `POST /api/upload` multipart route was removed; uploads now flow client → TOS directly via presigned PUT. The frontend calls `mediaAssets.getUploadUrl` (tRPC, `requireSession` via `protectedProcedure`) to mint a short-lived presigned URL + `object_key`, PUTs the bytes from the browser, then calls `mediaAssets.create` (or `assets.attachImage`) to persist the row. `mediaAssets.getUploadUrl` is the single upload contract — no other route writes binaries to TOS on behalf of the client. `@fastify/multipart` and the 50 MB in-memory buffer it required are gone; `nginx`'s `client_max_body_size` is back down to 2 m (tRPC payloads only).
    - **Verify before persist (BEN-27, 2026-06-08).** Signing the PUT with a server-authoritative ContentType (BEN-26) does **not** stop the client from PUTting arbitrary bytes — TOS records the signed header but never inspects the body. So before `mediaAssets.create` / `mediaAssets.createStandalone` / `assets.attachImage` insert a row, `services/upload/verifyObject.ts:verifyUploadedObject(objectKey, mediaType)` runs two probes against the just-uploaded object: (1) `storage.headObject` — the stored ContentType's family (`image|audio|video`) must equal `mediaType`; (2) `storage.getRange(0, 15)` + `detectMimeFromBytes` — magic bytes that actively detect a different family fail (null detection passes, matching `validate.ts`'s best-effort semantics). On any throw — verify failure or downstream DB failure — the mutation `storage.deleteObject(objectKey)` first (best-effort, swallowed) then rethrows, so a failed insert cannot leave an orphan blob in TOS. No size cap — bucket quota is the backstop (decision RF-?).
